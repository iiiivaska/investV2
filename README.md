...
Используется внешняя PostgreSQL база данных:
- **Хост**: 192.168.0.4
...
### Разработка (с локальной БД)
```bash
cd backend
./dev.sh
```

### Продакшн (с внешней БД)
```bash
cd backend
./deploy.sh
```
...
- FastAPI приложение (порт 8000)
- Redis (порт 6379)
- Внешняя PostgreSQL (192.168.0.4)

📖 **Подробное руководство по развертыванию**: [backend/DEPLOYMENT.md](backend/DEPLOYMENT.md)
⚡ **Быстрое развертывание**: [backend/QUICK_DEPLOY.md](backend/QUICK_DEPLOY.md)
